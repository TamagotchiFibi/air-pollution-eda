# 🌍 Анализ качества воздуха и уровня загрязнения в разных странах мира
...
на выявление наиболее загрязнённых регионов, определение наиболее часто встречающихся загрязнителей
и построение воспроизводимого аналитического pipeline: от очистки данных до
визуальных выводов.

## О проекте

EDA по данным о качестве воздуха в 23 000+ городах мира, направленный на проверку конкретных исследовательских вопросов, а не только визуальное изучение данных.
...
- определение наиболее часто встречающегося загрязнителя.
...
| **SQL (SQLite)** | агрегаты, группировки, CTE и оконные функции (`sql/queries.sql`) |
...
jupyter notebook notebooks/air_quality_eda.ipynb
```
...
репозиторий (`data/`) для воспроизводимости.

## Структура репозитория

```
air-pollution-eda/
├── notebooks/          # основной анализ (Jupyter)
├── sql/                # аналитические SQL-запросы (реальные, не заглушки)
├── images/             # экспортированные графики
├── data/               # датасет (air_pollution.csv)
├── requirements.txt
└── README.md
```
